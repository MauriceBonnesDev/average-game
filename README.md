...
Ausschließlich in Google Chrome getestet. Bitte Google Chrome verwenden!
...
5. #### Wechseln Sie ins Backend
   ...
   cd backend
   ```
6. #### Löschen der vorherigen Deployment ABIs
   ...
   ```bash
   rm -r ignition/deployments/chain-31337
   ```

   Windows

   ```bash
   rmdir ignition/deployments/chain-31337
   ```

7. #### Smart Contracts lokal deployen (Blockchain lokal muss laufen (Step 3))

   ```bash
   npx hardhat ignition deploy ./ignition/modules/migrationHardhat.ts --network localhost
   ```

8. #### Wechseln Sie ins Frontend

   ```bash
   cd ../frontend
   ```

9. #### Umbenennen der .json Dateien
...
10. #### Erstellen der Typescript Typen für die Contracts
    ```bash
